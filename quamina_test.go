...
func TestCityLots(t *testing.T) {
	...
	matcher := q.matcher.(*coreMatcher)
	fmt.Println(matcherStats(matcher))
	...
	for _, line := range lines {
		fields, err := fj.Flatten(line, matcher)
		if err != nil {
			t.Error("Flatten: " + err.Error())
		}

		matches, err := matcher.matchesForFields(fields)
		if err != nil {
...
func TestCityLots_JX(t *testing.T) {
	patterns := []string{
		`{ "properties": { "STREET": [ "CRANLEIGH" ] } }`,
		`{ "properties": { "STREET": [ "17TH" ], "ODD_EVEN": [ "E"] } }`,
		`{ "geometry": { "coordinates": [ 37.807807921694092 ] } }`,
	...
	fj := newJxFlattener(buildPathsIndex(matcher))
