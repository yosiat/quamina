...
func newJxFlattener(paths pathsIndex) *jxFlattener {
	return &jxFlattener{
		paths:      paths,
		fields:     make([]Field, 0),
		arrayTrail: make([]ArrayPos, 0),
		arrayCount: 0,
	}
}

func (fj *jxFlattener) Copy() Flattener {
	return &jxFlattener{
		paths:      fj.paths,
		fields:     make([]Field, 0),
		arrayTrail: make([]ArrayPos, 0),
...
func (fj *jxFlattener) Flatten(event []byte, tracker NameTracker) ([]Field, error) {
	fj.reset()
...
func (fj *jxFlattener) enterArray() {
	fj.arrayCount++
	fj.arrayTrail = append(fj.arrayTrail, ArrayPos{Array: fj.arrayCount, Pos: 0})
}
